...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!hasRole && !isAdmin) {
      return interaction.reply("<:xMark:1502740326668828703> You must be an employee to use this command.");
    }

    if (!channel.topic || !/^\d+(\|\d+)?$/.test(channel.topic)) {
      return interaction.reply({
        content: "<:xMark:1502740326668828703> This command must be used in a ticket.",
        flags: 64
      });
    }
    ...
    if (sub === "add") {
      ...
      if (!user && !role) {
        return interaction.reply({
          content: "<:xMark:1502740326668828703> Failed to detect a valid user or role.",
          flags: 64
        });
      }
